...
kable(head(edx_genres_ungroup,5), caption="*Tidy* format of the training and validation datasets.") %>% kable_styling(latex_options = "hold_position")
```
...
edx %>% group_by(movieId) %>% summarize(N=n(), avg=mean(rating)) %>% 
...
edx_genres %>% select(N) %>% OneR::bin(nbins=6) %>% 
  set_names("Bins") %>% cbind(edx_genres) %>% 
...
Converting theory into code gives the following function that calculates the $RMSE$ from the described model. Although this function is the final estimate, I show how adding each term reduces the $RMSE$ in the **Results** section.  
...
sapply(lambdas, function(l){
    ...
    b_g <- edx_genres_ungroup %>% 
      left_join(b_i, by="movieId") %>%
      left_join(b_u, by="userId") %>%
      group_by(genres) %>%
      summarize(b_g = sum(rating - b_i - b_u - overall)/(n()+l))
    b_t <- edx_genres_ungroup %>%  #non-regularized time effect calculation. 
      left_join(b_i, by="movieId") %>%
      left_join(b_u, by="userId") %>%
    ...
    preds <-validation_genres_ungroup %>% 
...
## Results & Discussion
```{r all_models}
error <- data.frame("Method"="Global Average","RMSE"=RMSE(plyr::round_any(overall,0.5),validation$rating))
...
error<-rbind(error,data.frame("Method"="Movie (R)","RMSE"=RMSE(predictions,validation$rating))) #error table of predictions vs. validation dataset
...
```
An optimal value of $\lambda$ was calculated for each model containing successively more $b$ terms. Table 3, below, shows the $RMSE$ output of each model with "(R)" indicating regularization. The $RMSE$ decreases for every additional $b$ term.
```{r error_table}
